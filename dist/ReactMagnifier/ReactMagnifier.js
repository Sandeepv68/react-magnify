...
import { isValidProp, logMagnifierError, triggerCustomEvent, getCursorPos, createMagnifierGlass, } from './utils.ts';
import './style.css';
/**
 * Constants
 */
 ...
 * />
 */
const ReactMagnifier = React.memo(function ReactMagnifier(props = {}) {
    // Merge props with defaults
    const finalProps = useMemo(() => (Object.assign(Object.assign({}, defaultProps), props)), [props]);
    // Refs
    ...
    const handleMoveMagnifier = useCallback((event) => {
        ...
        glass.style.backgroundPosition =
            `-${x * finalProps.zoomSize - width + PIXEL_PADDING}px -${y * finalProps.zoomSize - height + PIXEL_PADDING}px`;
    ...
    return (_jsxs("div", { className: `react-magnifier-image-container ${finalProps.customContainerStyles}`, ref: imageContainerRef, role: "group", "aria-label": "Image magnifier", tabIndex: 0, children: [_jsx("img", { ref: magnifiableImageRef, className: finalProps.customImgStyles, src: finalProps.imageUrl, width: finalProps.imageWidth, height: finalProps.imageHeight, alt: finalProps.imageAltText, role: "img", "aria-describedby": "magnifier-help" }), isMagnifierVisible && (_jsx("div", { id: "magnifier-help", className: "sr-only", role: "status", "aria-live": "polite", children: "Magnifier active. Use arrow keys to navigate, Escape to close." }))] }));
});
